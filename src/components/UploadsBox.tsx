        ...
        <>
            ...
            <div className="mt-3 grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-10 gap-2">
                {previewFiles.map((file, index) => {
                    let url = ["image/png", "image/jpeg", "image/jpg"].includes(file.type) ? URL.createObjectURL(file) : ZipIcon;
                    return (
                        <div className="flex gap-1 flex-col">
                            <div key={index} className="relative flex-1 cursor-pointer flex items center justify-center rounded-xl border border-gray-500 border-dashed group">
                                <div className="h-full flex items center justify-center p-1">
                                    <img src={url} alt="preview" className="rounded-xl object-contain" />
                                </div>
                                <button
                                    onClick={() =>
                                        onRemove(index)
